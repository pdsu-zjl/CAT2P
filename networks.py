...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        # self.x = torch.nn.Parameter(torch.Tensor([0]))  #实现参数添加
        self.fuse_weight_1 = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.fuse_weight_1.data.fill_(0.10)
        ...
        #在这里设计参数的添加问题  设计参数  带入损失过程 然后在处理得到结果


    def forward(self, input,name):#,size1,size2
        out = {}
        #input=getLBP(input)
        ...
        # out['u3']= self.up2(out['u1'])* self.fuse_weight_1self.fuse_weight_1
        # self.fuse_weight_1=F.sigmoid(self.fuse_weight_1)
        # out['u2']=self.outc(out['u2']).resize((1,3,size1, size2), Image.BICUBIC)
        if(name=='weight'):
          return F.sigmoid(self.fuse_weight_1)
        else:
